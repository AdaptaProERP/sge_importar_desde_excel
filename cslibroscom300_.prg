...
//                 (DR20101015a)Se incorporan cambios: AG20080618,DR20080623c,DR20090817 y DR20080623a que están asociados al Reintegro de Compras
...
PROCE MAIN(oCom)
   ...
   LOCAL cSqlR:="",oTabR
   LOCAL nNetoRei,oSumRei // DR20090817
   cClip:=""
   cClipSuma:=""
   cSuma27:=""
   nUltCol:=1
...
******indica si se incluye el rif en encabezado
   ...
   IF oCom:nCForm=1 .OR. oCom:nCForm=3 .OR. oCom:nCForm=4
      ...
      AADD(aCabecera,Padc("Comprobante",11)) // DR20110606a space(1))
...
//      ENDIF
      ...
      AADD(aCabecera,space(15))
      AADD(aCabecera,space(oCom:nCLenRs))
      AADD(aCabecera,PADC("Retención",11)) // DR20110606a space(1))
...
//      ENDIF
      ...
      AADD(aCabecera,space(11)) // DR20110606a space(1))
...
****** PARA QUE FUNCIONE CORRECTAMENTE
      ...
      cSql:="SELECT IF(DOC_ESTADO='NU',0,DOC_NETO) AS DOC_NETO,"+;
            ...
            " AND " + GetWhereAnd("DOC_FCHDEC",dFchCount,dFchCount)+;
      ...
      WHILE !oTable:Eof() 
         IF Empty(oTable:MOV_TIPIVA) .AND. oTable:DOC_TIPDOC<>'RTI' // No tiene Productos
            ...
            EJECUTAR("DPDOCCLIIVA",oTable:DOC_CODSUC,oTable:DOC_TIPDOC,oTable:DOC_CODIGO,oTable:DOC_NUMERO,.F.,;
                                  oTable:DOC_DCTO  ,oTable:DOC_RECARG,oTable:DOC_OTROS,NIL,"C")
...
*****XYZ
      ...
      // DR20101015a. Cambios Reintegros de Compras
      *****AG20080618
      // Lectura de Gastos Reembolsables
      // DR20080623c
      // DR20090817. Se agrega CD_CODIGO, CCD_TIPDOC, CCD_NUMERO
      cSqlR:=" SELECT CCD_TIPDOC,CCD_CODIGO,CCD_NUMERO,CCD_ACT,TDC_LIBTRA,CCD_FECHA,CCD_TIPIVA,CCD_CODSUC,CCD_RIF,CCG_NOMBRE,CCD_DOCTIP,CCD_FACTUR,CCD_NUMFIS,CCD_PORIVA,SUM(CCD_MONTO) AS CCD_BASE FROM DPDOCPROCTA "+;
             ...
             " INNER JOIN DPDOCPRO        ON CCD_CODSUC=DOC_CODSUC AND CCD_TIPDOC=DOC_TIPDOC AND CCD_CODIGO=DOC_CODIGO AND CCD_NUMERO=DOC_NUMERO AND DOC_ACT=1 AND DOC_TIPTRA='D'"+;
             " INNER JOIN DPTIPDOCPRO     ON CCD_DOCTIP=TDC_TIPO "+;
             " WHERE "+GetWhereAnd("CCD_FCHDEC",dFchCount,dFchCount)+; 
      ...
      WHILE !oTabR:Eof()
        // DR20090817. Lo buscamos en la tabla
        nNetoRei:=0 // DR20090817
        oSumRei:=OpenTable("SELECT SUM(CCD_MONTO+(CCD_MONTO*(CCD_PORIVA/100))) AS CCD_BASE FROM DPDOCPROCTA "+;
                           "WHERE CCD_CODSUC"+GetWhere("=",oCom:cCodSuc)+;
                           " AND CCD_TIPDOC"+GetWhere("=",oTabR:CCD_TIPDOC)+" AND CCD_CODIGO"+GetWhere("=",oTabR:CCD_CODIGO)+" AND CCD_NUMERO"+GetWhere("=",oTabR:CCD_NUMERO)+;
                           " AND CCD_TIPTRA='D' AND CCD_RIF"+GetWhere("=",oTabR:CCD_RIF)+" AND CCD_FACTUR"+GetWhere("=",oTabR:CCD_FACTUR)+" AND CCD_DOCTIP"+GetWhere("=",oTabR:CCD_DOCTIP))


        IF oSumRei:RecCount()>0
           nNetoRei:=oSumRei:CCD_BASE
        ENDIF
        ...
        ELSE
           oTable:REPLACE("TDC_LIBTRA","01-REG") // DR20080623b
        ENDIF
        ...
        oTable:REPLACE("DOC_DCTO",0) // DR20090817 BUG: oTabR:DOC_DCTO)
        ...
        oTable:REPLACE("DOC_NETO",nNetoRei) // DR20090817
...
**    oTable:REPLACE("RTI_FECHA",oTabR:CCD_BASE)
...
// oTable:Browse()
     oTabR:End()
     *****AG20080618
     oTable:GoTop() // DR20080623a

      DO WHILE !oTable:Eof()

*****XYZ
     IF oTable:MOV_IVA=0 .AND. NREGISTROS(oTable:DOC_DOCORG,oTable:DOC_CODIGO,oTable:DOC_NUMERO,oTable:DOC_CODSUC,oTable:DOC_TIPDOC)>0
		oTable:DbSkip()
		LOOP
	ENDIF
         ...
         cNumRti:=IIF(EMPTY(oTable:RTI_DOCNUM).OR.lRti,"",oTable:RTI_NUMTRA) // DR20110606c. cNumRti:=IIF(EMPTY(oTable:RTI_DOCNUM).OR.lRti,"",oTable:RTI_DOCNUM)
         nIvaRetOri:=0
         dFchRti:=CTOD("")
         IF !EMPTY(cNumRti)
...
******AG20080312 SE COLOCO NIVA <>0
         ...
         IF oCom:nCForm=1 .OR. oCom:nCForm=3 .OR. oCom:nCForm=4
            ...
            cClip:=cClip+IIF(EMPTY(cNumRti),"",STRZERO(YEAR(dFechaRet),4)+STRZERO(MONTH(dFechaRet),2))+cNumRti+CHR(9) // DR20110606a cClip:=cClip+" "+CHR(9)
...
//IIF( nBase<>0,"" ,IIF(oCom:lCColRti,"",CHR(13)) )
         ...
         *oHoja:Range( cRange1 ):Borders():LineStyle  := 1
         ...
         IF oCom:nCForm=1 
            ...
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIAR"'+oCom:cESepLis+oHoja:Name+'!U'+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)
            ...
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIAR"'+oCom:cESepLis+oHoja:Name+'!V'+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)+CHR(13)
         //
         ENDIF
         IF oCom:nCForm=3
            ...
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIAG"'+oCom:cESepLis+oHoja:Name+'!S'+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIGA"'+oCom:cESepLis+oHoja:Name+'!S'+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIAR"'+oCom:cESepLis+oHoja:Name+'!S'+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIAG"'+oCom:cESepLis+oHoja:Name+'!T'+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIGA"'+oCom:cESepLis+oHoja:Name+'!T'+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIAR"'+oCom:cESepLis+oHoja:Name+'!T'+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)+CHR(13)
         ENDIF
         IF oCom:nCForm=4
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIAG"'+oCom:cESepLis+oHoja:Name+'!'+cLetraBase+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIGA"'+oCom:cESepLis+oHoja:Name+'!'+cLetraBase+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)
            ...
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIAG"'+oCom:cESepLis+oHoja:Name+'!Y'+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(A'+ALLTRIM(STR(nRow))+'="VIGA"'+oCom:cESepLis+oHoja:Name+'!Y'+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)
            cClipSuma:=cClipSuma+'=+'+oCom:cEFunSi+'(AA'+ALLTRIM(STR(nRow))+'="VIAR"'+oCom:cESepLis+oHoja:Name+'!Y'+ALLTRIM(STR(nRow))+oCom:cESepLis+'0)'+CHR(9)
   ...
   IF oCom:nCForm=1 .OR. oCom:nCForm=3 .OR. oCom:nCForm=4
      ...
      IF oCom:nCForm=3
         ...
         oHoja:Cells( nRow, 18):Set( "NumberFormat", cMascMon )
         cRange2:="S"+ALLTRIM(STR(INT(nRowIni)))+":S"+ALLTRIM(STR(INT(nRowFin)))
         oHoja:Cells( nRow, 19):Value := IIF( lPrimera,0 ,"=+"+oCom:cEFunSum+"("+cRange2+")")
      ...
      IF oCom:nCForm=4
         ...
         oHoja:Cells( nRow, 25):Value := IIF( lPrimera,0 ,"=+"+oCom:cEFunSum+"("+cRange2+")")
         oHoja:Cells( nRow, 25):Set( "NumberFormat", cMascMon )
         cRange2:="Z"+ALLTRIM(STR(INT(nRowIni)))+":Z"+ALLTRIM(STR(INT(nRowFin)))
         oHoja:Cells( nRow, 26):Value := IIF( lPrimera,0 ,"=+"+oCom:cEFunSum+"("+cRange2+")")
...
****** PARA QUE FUNCIONE CORRECTAMENTE
         ...
         IF oCom:nCForm=4
            ...
            oHoja2:Cells( nRow, 24):Set( "NumberFormat", cMascMon )
            cRange2:="Y"+ALLTRIM(STR(INT(nRowIni)))+":Y"+ALLTRIM(STR(INT(nRowFin)))
            oHoja2:Cells( nRow, 25):Value := IIF( .F. ,0 ,"=+"+oCom:cEFunSum+"("+cRange2+")")
            oHoja2:Cells( nRow, 25):Set( "NumberFormat", cMascMon )
            cRange2:="Z"+ALLTRIM(STR(INT(nRowIni)))+":Z"+ALLTRIM(STR(INT(nRowFin)))
            oHoja2:Cells( nRow, 26):Value := IIF( .F. ,0 ,"=+"+oCom:cEFunSum+"("+cRange2+")")
